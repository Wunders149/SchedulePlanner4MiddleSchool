...
import { Plus, X, Pencil, Trash2, Users, Loader2, BookOpen } from "lucide-react";
...
export default function ScheduleLedger() {
  ...
  const openEditor = (day, grade, periodId) => {
    ...
    setDraftActivity(existing?.activity || "");
  ...
  const saveEntry = () => {
    if (!draftActivity.trim()) {
      setError("Give this class an activity name first.");
      return;
    }
    const key = cellKey(editing.day, editing.grade, editing.periodId);
    const next = {
      ...entries,
      [key]: { activity: draftActivity.trim(), teachers: draftTeachers },
    };
    setEntries(next);
    persist(next);
    closeEditor();
  ...
  return (
    ...
    >
      <style>{`
        ...
        input:focus { outline: 2px solid #D98E2B; outline-offset: 1px; }
                    ...
                    return (
                      <tr key={period.id}>
                        <td
                          ...
                          style={{
                            padding: "8px 16px",
                            background: "repeating-linear-gradient(135deg, #F0ECDF, #F0ECDF 6px, #E8E2D0 6px, #E8E2D0 12px)",
                          ...
                          >
                            <div
                              ...
                              style={{
                                ...
                                borderRadius: 8,
                                padding: "10px 12px",
                                background: entry ? `${colorFor(entry.activity)}14` : "#F6F3EA",
                                border: entry ? `1px solid ${colorFor(entry.activity)}55` : "1px dashed #D8D2C2",
                              }}
                            >
                              {entry ? (
                                <>
        ...
        >
          ...
          >
            ...
            <div className="lp-body" style={{ marginTop: 18 }}>
              ...
              {error && (
                <div style={{ color: "#C1584A", fontSize: 12.5, marginTop: 12 }}>{error}</div>
              )}
